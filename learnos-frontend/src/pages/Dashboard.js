...
function ProgressRing({ value, size=64, stroke=5, color='#7A9E87' }) {
  ...
  return (
    <svg width={size} height={size} style={{ flexShrink:0 }}>
      ...
      <text x="50%" y="50%" textAnchor="middle" dy="0.35em" fontSize={size*0.22} fill="currentColor" fontFamily="inherit" fontWeight="600">{value}%</text>
    </svg>
  );
}
...
export default function Dashboard({ user, refreshKey }) {
  ...
  const typeLabels = { DEADLINE:'⏰ Deadline Alert', FORGETTING:'🧠 Forgetting Alert', PLATEAU:'📊 Mastery Plateau', STREAK:'🔥 Streak', AVOIDANCE:'⚠ Avoidance', GOAL_RISK:'🎯 Goal At Risk', MOTIVATION:'💬 Motivation', INFO:'ℹ Info' };
    ...
    <div className="fade-up">
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1.5fr 1.2fr', gap:20, marginBottom:24 }}>
        <div className="lei-card fade-up">
          ...
          <div style={{ marginTop:12 }}>
            <span className={`lei-risk ${riskClass}`}>{lei.score===0?'→ Log sessions to score':`Risk: ${lei.risk}`}</span>
          </div>
        </div>
        ...
        <div className="card fade-up" style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div className="stat-chip"><div className="stat-chip-value">{goals.length}</div><div className="stat-chip-label">Active Goals</div></div>
          <div className="stat-chip"><div className="stat-chip-value">{goals.reduce((s,g)=>s+(g.topics?.length||0),0)}</div><div className="stat-chip-label">Total Topics</div></div>
        </div>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1.2fr', gap:20, marginBottom:24 }}>
        <div className="card fade-up">
          ...
          <div style={{ marginTop:16, fontSize:12, color:'var(--text-muted)', fontStyle:'italic' }}>Scores update after each logged session</div>
        </div>
        <div className="fade-up">
