  ...
  if (phase === 'success') {
    return (
      <div className="fade-up" style={{ textAlign:'center', padding:'60px 20px', maxWidth:500, margin:'0 auto' }}>
        ...
        <h2 style={{ fontFamily:'var(--font-display)', fontSize:28, color:'var(--text-primary)', marginBottom:8 }}>Session logged!</h2>
        ...
        <div style={{ background:'white', borderRadius:16, padding:24, boxShadow:'0 2px 20px rgba(27,42,74,0.08)', marginBottom:16 }}>
          <div style={{ fontFamily:'var(--font-display)', fontSize:18, color:'var(--text-primary)', marginBottom:8 }}>Test your knowledge?</div>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={() => setPhase('quiz')} style={{ flex:1, padding:'13px', borderRadius:12, border:'none', background:'#1B2A4A', color:'white', fontSize:14, fontFamily:'inherit', fontWeight:600, cursor:'pointer' }}>Take Quiz ✦</button>
            <button onClick={resetForm} style={{ flex:1, padding:'13px', borderRadius:12, border:'1.5px solid rgba(27,42,74,0.15)', background:'transparent', color:'#5A6A80', fontSize:14, fontFamily:'inherit', cursor:'pointer' }}>Skip for now</button>
          </div>
        </div>
        <button onClick={resetForm} style={{ border:'none', background:'none', color:'var(--text-muted)', fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>Log another session</button>
      </div>
    );
  }
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:20 }}>
            {MASTERY_LEVELS.map(m => (
              <button key={m} onClick={() => setForm({...form, masteryBefore:m})}
                style={{ padding:'8px 14px', borderRadius:20, border:'none', cursor:'pointer', fontSize:12, fontWeight:600, transition:'all 0.2s', fontFamily:'inherit', background:form.masteryBefore===m?MASTERY_COLORS[m]:'var(--cream-dark)', color:form.masteryBefore===m?'white':'var(--text-muted)' }}>
                {m}
              </button>
            ))}
          </div>
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {MASTERY_LEVELS.map(m => (
              <button key={m} onClick={() => setForm({...form, masteryAfter:m})}
                style={{ padding:'8px 14px', borderRadius:20, border:'none', cursor:'pointer', fontSize:12, fontWeight:600, transition:'all 0.2s', fontFamily:'inherit', background:form.masteryAfter===m?MASTERY_COLORS[m]:'var(--cream-dark)', color:form.masteryAfter===m?'white':'var(--text-muted)' }}>
                {m}
              </button>
            ))}
          </div>
