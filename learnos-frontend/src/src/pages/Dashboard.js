...
import { mockInsights, mockLEI } from '../api/data';
  ...
  return (
    <div className="fade-up">
      <div className="page-header">
        ...
        <p className="page-subtitle">{goals.length} active goal{goals.length !== 1 ? 's' : ''} · Log a session to start scoring</p>
      </div>
        ...
        <div className="lei-card fade-up">
          ...
          <div style={{ marginTop: 12 }}>
            <span className={`lei-risk ${riskClass}`}>
              {goals.length === 0 ? '→ Add goals to begin' : '→ Log sessions to score'}
            </span>
          </div>
        </div>
